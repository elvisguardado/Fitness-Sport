...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (hour < 12) {
        ...
        greeting = 'Buenas tardes' + <b>' . $_SESSION['alias_usuario'] . '</b>;
    } else if (hour <= 23) {
...
function graficaBarrasMarca() {
    fetch(API_PRODUCTOS + 'cantidadProductosMarca', {
        method: 'get'
        ...
        if (request.ok) {
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    barGraph('chart1', marca, cantidad, 'Cantidad de productos', 'Cantidad de productos por categoría');
                } else {
        ...
        if (request.ok) {
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    pieGraph('chart2', marca, cantidad, 'Porcentaje de productos por categoría');
                } else {
