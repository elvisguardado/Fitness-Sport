...
require_once('../../models/detalle_compra.php');
...
if (isset($_GET['action'])) {
    ...
    if (isset($_SESSION['id_usuario'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'readAll':
                if ($result['dataset'] = $detalle_compra->readAll()) {
                    $result['status'] = 1;
                } else {
            ...
            case 'search':
                $_POST = $detalle_compra->validateForm($_POST);
                if ($_POST['search'] != '') {
                    if ($result['dataset'] = $detalle_compra->searchRows($_POST['search'])) {
                        $result['status'] = 1;
            ...
            case 'readOne':
                if ($detalle_compra->setId($_POST['id_detalle_compra'])) {
                    if ($result['dataset'] = $detalle_compra->readOne()) {
                        $result['status'] = 1;
                    } else {
                        if (Database::getException()) {
                            $result['exception'] = Database::getException();
